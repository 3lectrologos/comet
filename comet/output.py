...
def output_comet_viz(args, mutations, resultsTable, maxPermutedWeight, permutedN):
    ...
    if len(edges) == 0: # no significant results
        ...
        ccs = []
        print(f'Connected components: {ccs}')
        import pickle
        with open(os.path.join(args.output_directory, 'ccs.pcl'), 'wb') as fout:
            pickle.dump(ccs, fout)
        exit(0)
    ...
    ccomp = nx.connected_components(MPG)
    ccs = [list(cc) for cc in ccomp if len(cc) > 1]
    print(f'Connected components: {ccs}')
    import pickle
    print('OUT DIR:', os.path.join(args.output_directory, 'ccs.pcl'))
    with open(os.path.join(args.output_directory, 'ccs.pcl'), 'wb') as fout:
        pickle.dump(ccs, fout)
...
def choose_delta( deltas, realDist, expectedPoint, stdCutoff):
    ...
    while True:
        ...
        thisSlope = (logY[i] - logY[iS]) / (logX[i] - logX[iS])
        if lastSlopeI is not None and lastSlope > thisSlope:
            break
        elif lastSlopeI is None:
            lastSlope = thisSlope
            lastSlopeI = iS
    ...
    #import matplotlib.pylab as plt
    #plt.plot(logX, [np.log10(expectedPoint)]*len(logX), 'k--')
    #plt.plot(logX, logY, 'bo-')
    #plt.plot(logX[lastSlopeI], logY[lastSlopeI], 'rx', markeredgewidth=10)
    #plt.show()
